...
// import AssignmentData from "../../../Data/DataTable/AssignmentData";
...
const Assignment = () => {
  ...
  const [idAssignment, setIdAssignment] = useState(0);
  ...
  const assignmentRes = (id) => {
    localStorage.setItem("assignment-id", id);
    navigate('result');
  }
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:8000/api/assignment", {user})
      .then((res) => {
        setAssignmentData(res.data);
      })
      .catch((err) => {
        setAssignmentData({ message: "get data failed" });
      });
  }, []);
  ...
  const updateData = (id) => {
    axios
      .post("http://localhost:8000/api/assignment/" + id)
      .then((res) => {
        setAssignment({
          ...assignment,
  ...
  const createAssignment = () => {
    ...
    const { id_subject, title, description } = assignment;
    ...
    const start_at = moment(startAt, "YYYY-MM-DD, HH:mm").format(
      "DD-MM-YYYY HH:mm"
    );
    const end_at = moment(endAt, "YYYY-MM-DD, HH:mm").format(
      "DD-MM-YYYY HH:mm"
    );
    ...
    for (let i = 0; i < checkedItems.length; i++) {
      formData.append("checkedItems[]", checkedItems[i]);
    }
    axios
      .post("http://localhost:8000/api/assignment/data/add", formData)
      .then((res) => {
  ...
  const updateAssignment = () => {
    const id_teacher = localStorage.getItem("user-id");
    const { id_subject, title, description, start_at, end_at } = assignment;
    axios
      .post("http://localhost:8000/api/assignment/data/add", {
        id_teacher,
        id_subject,
        title,
        description,
  ...
  const deleteAssignment = () => {
    const id = idAssignment;
    axios
      .post("http://localhost:8000/api/assignment/delete/" + id)
      .then((res) => {
  ...
  const handleFileChange = (event) => {
    ...
    // Di sini Anda dapat mengakses informasi file seperti nama file, tipe file, ukuran file, dll.
    console.log("File Name:", file.name);
    console.log("File Type:", file.type);
    console.log("File Size:", file.size);
  };

  const AssignInputChildren = (
    <div className="w-[95vw] h-[80vh]">
      ...
      <div className="flex items-center">
        <div className="relative min-w-[30rem]">
          <textarea
            className="peer h-auto min-h-[100px] w-[30rem] resize-none border-b border-blue-gray-200 bg-transparent pt-4 pb-1.5 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border-blue-gray-200 focus:border-pink-500 focus:outline-0 disabled:resize-none disabled:border-0 disabled:bg-blue-gray-50"
            placeholder=" "
            name="description"
        ...
        <div className="flex items-center">
          <input
            type="file"
            class="block w-[22rem] mx-3 text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-violet-50 file:text-violet-70 hover:file:bg-violet-100 border border-slate-700 rounded-lg mt-4 py-1"
            //   value={selectedFile}
            name="content"
      ...
      <div className="mt-4">
        ...
        <div className="relative w-full min-w-[200px] flex items-center">
          ...
          <div className="text-center">
            ...
            <TimePicker
              onChange={({ target }) => changeTime(target)}
              hourName="hour_start"
              minuteName="minute_start"
              hour={time.hour_start}
              minute={time.minute_start}
            />
          </div>
          <div className="text-center">
            <label>End At</label>
            <TimePicker
              onChange={({ target }) => changeTime(target)}
              hourName="hour_end"
              minuteName="minute_end"
              hour={time.hour_end}
              minute={time.minute_end}
            />
          </div>
        </div>
      </div>
    ...
    <div className="max-w-[95vw] pb-6 pr-5 flex">
      ...
      <div className="w-full h-auto mt-1 ml-16">
        <div className="w-[94vw] h-28 bg-third rounded-t-lg mt-4 pl-5 pt-5">
          <p className="font-bold text-3xl text-white">Assignment List</p>
        </div>
        {assignmentData.length > 0 ? renderCard(assignmentData) : cardNoData()}
      </div>
      <div className="w-screen">
        <Modal
          open={insertModal}
          onClose={() => setInsertModal(false)}
          children={AssignInputChildren}
        />
        <Modal
          open={manageModal}
          onClose={() => setManageModal(false)}
          children={AssignManageChildren}
        />
        <Modal
          open={deleteModal}
          onClose={() => setDeleteModal(false)}
          children={AssignDeleteChildren}
        />
      </div>
    </div>
  );
