...
// import LessonData from "../../../Data/DataTable/LessonData";
...
const Lesson = () => {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:8000/api/lesson", { user })
      .then((res) => {
        setLessonData(res.data);
      })
      .catch((err) => {
        setLessonData({ message: "get data failed" });
      });
  }, []);
  ...
  const updateData = (id) => {
    axios
      .post("http://localhost:8000/api/lesson/" + id)
      .then((res) => {
        setLesson({
          ...lesson,
  ...
  const createLesson = () => {
    ...
    const { id_subject, title, description } = lesson;
    ...
    axios
      .post("http://localhost:8000/api/lesson/data/add", formData)
      .then((res) => {
        console.log(res);
  ...
  const updateLesson = () => {
    const id_teacher = localStorage.getItem("user-id");
    const { id_subject, title, description, start_at, end_at } = lesson;
    axios
      .post("http://localhost:8000/api/lesson/data/add", {
        id_teacher,
        id_subject,
        title,
        description,
  ...
  const deleteLesson = () => {
    const id = idLesson;
    axios
      .post("http://localhost:8000/api/lesson/delete/" + id)
      .then((res) => {
  ...
  const LessInputChildren = (
    <div className="w-[95vw] h-[80vh]">
      <h1 className="mb-8 text-2xl font-bold">Lesson</h1>
      ...
      <div className="relative w-full min-w-[200px] mt-4">
        ...
        <div className="flex">
          <select
            id="subject"
            name="id_subject"
            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-96 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mt-4"
    ...
    <div className="max-w-[95vw] pb-6 pr-5 flex">
      ...
      <div className="w-full h-auto mt-1 ml-16">
        <div className="w-[94vw] h-28 bg-third rounded-t-lg mt-4 pl-5 pt-5">
          <p className="font-bold text-3xl text-white">Lesson List</p>
        </div>
        {lessonData.length > 0 ? renderCard(lessonData) : cardNoData()}
      </div>
      <div className="w-screen">
        <Modal
          open={insertModal}
          onClose={() => setInsertModal(false)}
          children={LessInputChildren}
        />
        <Modal
          open={manageModal}
          onClose={() => setManageModal(false)}
          children={LessManageChildren}
        />
        <Modal
          open={deleteModal}
          onClose={() => setDeleteModal(false)}
          children={LessDeleteChildren}
        />
      </div>
    </div>
  );
